...
@Inject constructor(
    ...
    override suspend fun onExecute(request: ParkingSpotRequest): ParkingValidationStatus = when {
        ...
        request.parkingTimeFormatted.isEmpty() -> ParkingValidationStatus.Failure.InvalidParkingTime(
            resources
        )
        ParkingType.exists(request.parkingType).not() ->
    ...
    private fun validateParkingLotInformation(request: ParkingSpotRequest): ParkingValidationStatus {
        ...
        val parkingType = ParkingType.forValue(request.parkingType)!!
        return if (parkingType == ParkingType.ParkingLot) {
            when {
                ParkingFloorType.exists(request.floorType).not() ->
